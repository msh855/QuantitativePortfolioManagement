...
def bootstrap_stats(returns:pd.Series, 
    ...
    if isinstance(returns_benchmark, pd.Series)==False:
    ...
    if returns_benchmark.empty ==False:
...
def bootstrap_portfolio_performance(returns: pd.Series,
    ...
    if isinstance(returns_benchmark, type(None)) == False:
...
def prep_dist(x: pd.DataFrame,  name_perc:list = ['0.05', '0.20','0.35','0.65','0.80', '0.95']) -> pd.DataFrame:
    """
...
def backtest_report(returns:pd.DataFrame, 
    ...
    if out_of_sample_date:
        ...
        df2 = df_out_of_sample.transpose().style.set_table_attributes("style='display:inline'").set_caption('Out Of Sample')
