...
def extract_implied_distribution(
    ...
    result = pd.DataFrame({"price_level": strikes_dense, "probability_density": density})
...
def bootstrap_future_distribution(
    returns: pd.Series, S0: float, T: float, n_sim: int = 10000, bootstrap_method: str = "iid", block_size: int = 20
) -> pd.DataFrame:
    """
    ...
    method_map = {"iid": "nbb", "block": "mbb", "stationary": "sb", "circular": "cbb"}

    bootstrap_type = method_map.get(bootstrap_method, "nbb")

    # Bootstrap returns
    bootstrapped_returns = bootstrappingTS(
        series=returns, block_size=block_size, n_samples=n_sim, bootstrap_type=bootstrap_type
    )
    ...
    return pd.DataFrame({"simulation": range(len(future_prices)), "future_price": future_prices.values})
...
def plot_distribution_comparison(
    ...
    ax1.hist(bootstrap_prices_plot, bins=bins, density=True, alpha=0.5, label=label, color="green", edgecolor="black")
...
def find_mispricing_opportunities(comparison: pd.DataFrame, threshold_pct: float = 10.0) -> pd.DataFrame:
    """
    ...
    """
    mispricings = comparison[np.abs(comparison["pct_difference"]) > threshold_pct].copy()

    if len(mispricings) == 0:
        return pd.DataFrame(
            {"metric": ["No significant mispricing detected"], "opportunity": ["Markets appear fairly priced"]}
        )
