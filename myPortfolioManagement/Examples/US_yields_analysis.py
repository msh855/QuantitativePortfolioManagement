...
df_US_yield_factors = get_USyield_curve_factors(start_date= '1995-01-01', yields = ['3M','6M','1Y','2Y','3Y','5Y','7Y','10Y','20Y','30Y'], freq = 'm')
...
# get recessions 
US_rec = download_fred_data(['USREC'], freq = 'm')
...
# create plot
fig, ax = plt.subplots()
df_US_yield_factors[['USyc_ShiftFactor', 'USyc_slope', 'USyc_curvature', 'Date']].plot.line(ax=ax, figsize=(8, 5), x='Date', alpha=0.4, color="blue", label='Correlation')
...
plt.ylim(-1, 1.1)
plt.legend(["6 months rolling Correlations", '36 months rolling correlations', "US Recessions"], loc ="lower right", prop={'size': 8})
...
df_dummy = yields_monthly[['spread_10Y_3M', 'spread_10Y_2Y']].dropna()
s1 = df_dummy['spread_10Y_3M']
s2 = df_dummy['spread_10Y_2Y']
...
grangercausalitytests(yields_monthly[['spread_10Y_3M', 'spread_10Y_2Y']].dropna(), maxlag=[6])
grangercausalitytests(yields_monthly[['spread_10Y_2Y', 'spread_10Y_3M']].dropna(), maxlag=[6])
...
US_yields = US_yields[['spread_2Y_3M']].merge(yields_monthly[['spread_10Y_3M','spread_10Y_2Y' ]], left_index=True,right_index = True)

pd.to_csv(US_yields.dropna())
