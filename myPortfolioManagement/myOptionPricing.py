...
def option_delta(S: float, K: float, T: float, r: float, sigma: float, option_type: str = "call") -> float:
    """
...
def create_option_chain(
    S: float, T: float, r: float, sigma: float, strike_range: Tuple[float, float] = None, num_strikes: int = 20
) -> pd.DataFrame:
    """
    ...
    for K in strikes:
        ...
        option_data.append(
            {"strike": K, "call_price": call_price, "put_price": put_price, "call_iv": sigma, "put_iv": sigma}
        )
