...
def ts_clustering(
    df: pd.DataFrame, number_of_clusters: int = None, algo: str = ["dtw", "softdtw"][0], plot_bar_center: bool = True
):
    ...
    model = TimeSeriesKMeans(n_clusters=number_of_clusters, metric=algo, max_iter=100, random_state=0)
    ...
    df_clusters = (
        pd.DataFrame(zip(df.transpose().index, fancy_names_for_labels), columns=["asset", "Cluster"])
...
def detect_regimes(
    df: pd.DataFrame, series: str, optimal_clusters: int = 3, metric="dtw", plot_regimes=True, **kwargs
) -> pd.DataFrame:
    X = df[[series]]
    ...
    if plot_regimes:
        X.reset_index().plot.scatter(
            x=X.index.name, y=series, c=regime_name, cmap="viridis", figsize=(16, 10), sharex=False
        )

        plt.ylabel(series, fontsize=20)
        plt.xlabel("Year", fontsize=20)
        plt.legend(fontsize=15)
        plt.title("Regimes", fontsize=20)
...
def kMeansClusterSeries(df: pd.DataFrame, method="ward", metric="euclidean", title: str = "Dendrogram", **kwargs):
    ...
    dendrogram(hier_ward, labels=X_scaled.index.values, color_threshold=3, **kwargs)
...
def cluster_ftca(returns: pd.DataFrame, threshold: float = 0.70, col_name: str = None):
    ...
    df_clusters = (
        pd.DataFrame.from_dict(clust, orient="index").sort_index().stack().reset_index(level=1, drop=True).reset_index()
    )
