...
currencies = ['CHFUSD', 'EURUSD', 'JPYUSD', 'CADUSD', 'GBPUSD', 'NZDUSD', 'AUDUSD', 'NOKUSD', 'SEKUSD'][4]
...
data_normalised = remove_outliers(data_normalised)
...
# plot factors
# =============
df_spot_sm.join(df_factors[['factor1']]).plot(secondary_y='factor1')
...
# # factor 1 is the common factor capturing the trend in the risk adjust returns of the raw data
# # ============================================================================================
# df_factors[['factor1']].join(ret_cum_adj).plot(secondary_y='return_cum_risk_adj')
# df_factors[['factor1']].plot()


vol_scale = 0.075
for fact in df_factor_sm.columns.tolist():
    df_mom[fact] = np.where(df_mom[fact] > df_mom['factor1'], 1, -1)
    # df_mom[fact + '_pct'] = np.where(df_mom['factor1'] > 0, 1, -1)
    # df_mom[fact + '_pct'] = (df_mom[fact + '_pct'] / raw_vol['ret_vol']) * vol_scale
...
# df_stats = mainStats(df_performance.pct_change()['Prod_mom'])
# df_stats.columns = ['Prod']
# df_stats_fact = mainStats(df_performance[fact + '_adj'].pct_change())
# df_stats_fact.columns = [fact + '_adj']
# df_stats = df_stats.join(df_stats_fact)
# df_mom = df_mom.drop(df_factor_sm.columns.tolist(), axis=1)

#
# #df_mom['return_cum_risk_adj_norm'] = normalise_data(df_mom[['return_cum_risk_adj']]).iloc[:,0]
# df_factor_sm_2 = make_sma(df=df_mom[['return_cum_risk_adj_norm']], spans=[16, 32, 64, 128, 256])
# df_mom = df_mom.join(df_factor_sm_2)
#
# df_mom[['factor1', 'return_cum_risk_adj_norm']].plot(secondary_y='return_cum_risk_adj_norm')
# df_mom[['factor1', 'return_cum_risk_adj_norm']].plot()

# for fact in df_factor_sm.columns.tolist() + df_factor_sm_2.columns.tolist():
#     df_mom[fact] = np.where(df_mom[fact] > df_mom['factor1'], 1, -1)
#     df_mom[fact] = (df_mom[fact] / raw_vol['ret_vol']) * vol_scale
#     df_performance = get_PnL(df_mom, signals_list=[fact, 'Prod_mom'])
#     df_performance.plot(title = ccy + fact)
