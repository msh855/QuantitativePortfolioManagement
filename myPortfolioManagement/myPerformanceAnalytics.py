...
# def information_ratio(returns , benchmark:str, my_assets_col_name:str):
    
#     # need to check if date is in column or an index
    
#     returns_benchmark = returns[benchmark]
#     returns_non_benchmark = returns.drop([benchmark], axis = 1)
#     columns = returns_non_benchmark.columns.to_list()
    
#     info_ratio = []
#     for col in columns:
#         diff_rets = returns_non_benchmark[col] - returns_benchmark
#         diff_std = diff_rets.std()
#         inf_r = diff_rets.mean() / diff_std
#         info_ratio.append(inf_r)
     
#     # cleaning     
#     temp = pd.DataFrame(info_ratio)
#     temp[my_assets_col_name] = columns
#     temp.columns = ['information_ratio',my_assets_col_name]
#     temp= temp.set_index(my_assets_col_name)
#     return temp
...
def ranking_metrics(df, rolling_window = 3, 
  ...
  df_reward_metrics['overall_score'] = df_reward_metrics[colNames_cons_score] + df_reward_metrics[colNames_cons_alpha] + df_reward_metrics[colNames_reward]
